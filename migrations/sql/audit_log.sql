...
CREATE OR REPLACE FUNCTION "jsonb_minus"("json" jsonb, "keys" TEXT[])
  RETURNS jsonb
  LANGUAGE sql
  IMMUTABLE
  STRICT
...
COMMENT ON COLUMN audit.log.session_id IS 'User Session/Token which caused a modification';
...
BEGIN
  ...
  IF ((TG_OP = 'UPDATE' OR TG_OP = 'INSERT') AND TG_LEVEL = 'ROW') THEN
    ...
    IF json_new ? 'updated_session_id' THEN
      session_id = NEW.updated_session_id;
    END IF;
  END IF;
  ...
  IF (TG_OP = 'UPDATE' AND TG_LEVEL = 'ROW') THEN
    audit_row.original = json_old - excluded_cols;
    audit_row.diff = (json_new - audit_row.original) - excluded_cols;
  ...
  ELSIF (TG_OP = 'INSERT' AND TG_LEVEL = 'ROW') THEN
    audit_row.original = json_new - excluded_cols;
