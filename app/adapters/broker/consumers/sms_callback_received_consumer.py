"""
Sms Callback Received Consumer to handle consuming Submitted SMS Callback message events from broker
"""
...
class SmsCallbackReceivedConsumer(Consumer):
    """
    SMS Callback Received Consumer handle consuming SmsCallbackReceived events using a Kafka consumer to Kafka Broker
    Cluster
    """

    def __init__(self, kafka_consumer: KafkaConsumer):
        """
        Creates an instance of sms callback consumer to consumer events
        Args:
    ...
    def consume(self) -> Optional[SmsCallback]:
        """
        ...
        try:
            ...
            if self.message:
                data = self.message.sms_callback

                sms_id = UniqueId(data.id)
                sender = PhoneNumber(data.sender)
                recipient = PhoneNumber(data.account_sid)
